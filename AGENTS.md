...
Non-obvious caveats:
- **Aviationstack requires `AVIATIONSTACK_ACCESS_KEY`.** Without it the primary source returns an error and the app falls back to adsb.fi (or shows `0 bananas` / `Waiting for aircraft data…` if that also fails). Free-tier Aviationstack accounts may need `AVIATIONSTACK_BASE_URL=http://api.aviationstack.com/v1/flights` (HTTPS is paid-only on some plans).
- **No Cesium Ion token by default.** Without `VITE_CESIUM_ION_TOKEN` in `.env`, Cesium uses its default ion token / OSM fallback and the globe still renders (with a token warning in the overlay). `.env` is optional; copy `.env.example` to `.env` and set tokens as needed.
